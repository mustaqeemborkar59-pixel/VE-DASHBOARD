...
import { FileSpreadsheet, Printer, RefreshCw, Trash2, Zap, Ruler, Info, Loader2 } from 'lucide-react';
...
function ForkliftSheetContent() {
  const { toast } = useToast();
  const { firestore } = useFirebase();
  const searchParams = useSearchParams();
  const forkliftId = searchParams.get('id');
  ...
  useEffect(() => {
    const fetchForklift = async () => {
      if (!forkliftId || !firestore) return;
      
      setIsLoading(true);
      try {
        const docRef = doc(firestore, 'forklifts', forkliftId);
        const docSnap = await getDoc(docRef);
        
        if (docSnap.exists()) {
          const data = docSnap.data() as Forklift;
          setSheetData(prev => ({
            ...prev,
            ...
            liftHeight: data.mastHeight || '',
            remarks: data.remarks || '',
          }));
          toast({ title: "Unit Connected", description: `Data for ${data.serialNumber} has been loaded.` });
        } else {
          toast({ variant: 'destructive', title: "Not Found", description: "The requested forklift could not be found." });
        }
      } catch (err) {
        console.error(err);
        toast({ variant: 'destructive', title: "Connection Error", description: "Failed to load shared unit data." });
      } finally {
        setIsLoading(false);
      }
    };
    ...
    <div className="flex flex-col gap-6 max-w-6xl mx-auto animate-in fade-in duration-500 print:hidden">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground uppercase font-bold tracking-widest opacity-70">
            Professional Specification Generator
          </p>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          <Button variant="outline" onClick={handleClear} className="h-10 px-4 font-bold border-destructive/20 text-destructive hover:bg-destructive/5">
              <Trash2 className="mr-2 h-4 w-4" /> Clear Sheet
          </Button>
      ...
      {isLoading ? (
          <div className="flex flex-col items-center justify-center h-64 gap-4 text-muted-foreground">
              ...
              <p className="font-bold uppercase tracking-widest text-xs">Loading Shared Data...</p>
          </div>
      ) : (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <Card className="lg:col-span-5 border-none shadow-xl rounded-2xl bg-card">
              <CardHeader className="border-b pb-4">
                ...
                <CardDescription>Fill technical specs manually or load from fleet list.</CardDescription>
              </CardHeader>
